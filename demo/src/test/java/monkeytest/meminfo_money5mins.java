...
//测试初始化POS内存性能
public class meminfo_money5mins {
	int r=0;
	WriteExcelAndReadExcel W = new WriteExcelAndReadExcel();
	private String path = "";
	@Test(dataProvider = "meminfotest", description = "meminfotest for P1 monkeytest", enabled = true)
	public void meminfotest(Map<String, String> testdata) {
		...
		try {
			 ...
			 */
//			记录结果进TXT
			Runtime.getRuntime().exec("cmd /c E:&&cd E:\\tools\\adb\\adb&&" + commandsa + commandsb+".txt");
//			回写ＥＸＣＥＬ
